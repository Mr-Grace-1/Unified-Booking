...
import { Repeat, Calendar, Clock, Users, MapPin } from 'lucide-react';
import { useApp } from '../store/AppContext';
import { services, customers, staff, locations } from '../data/mockData';
import { useToast } from './Toast';
import { RecurringBooking } from '../types';

export default function BookingRecurrence() {
  const { recurringBookings, createRecurringBooking, deleteRecurringBooking } = useApp();
  ...
  const handleSubmit = () => {
    ...
    createRecurringBooking(newRecurring);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="p-4 rounded-xl bg-slate-900/50 border border-white/10">
          <div className="text-2xl font-bold text-white">{recurringBookings.length}</div>
          <div className="text-sm text-slate-400">Total Recurring</div>
        </div>
        <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
          <div className="text-2xl font-bold text-emerald-400">
            {recurringBookings.filter((r: RecurringBooking) => r.isActive).length}
          </div>
          <div className="text-sm text-slate-400">Active</div>
        </div>
        <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
          <div className="text-2xl font-bold text-blue-400">
            {recurringBookings.reduce((sum: number, r: RecurringBooking) => sum + r.occurrences, 0)}
          </div>
          <div className="text-sm text-slate-400">Total Occurrences</div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {recurringBookings.length === 0 ? (
          ...
          recurringBookings.map((recurring: RecurringBooking) => {
                ...
                <div className="flex items-start justify-between mb-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-2 text-xs text-slate-500">
                        <span className="flex items-center gap-1">
                          <Repeat size={12} />
                          {patternLabels[recurring.pattern as keyof typeof patternLabels]}
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 rounded text-xs ${
                      recurring.isActive ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-500/20 text-slate-400'
                    }`}>
